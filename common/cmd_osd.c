                         ...
                         U8 u8Pos_y)
{
    ...
    for(i = 0; i < num; i++)
    {
        PaintTextTransfer(LCDDisplay_Text[i], (char *)Text_Transfer);
        for(j = 0; j < 8; j++)
            for(k = 0; k < 5; k++)
...
{
    BootTEXTStr((unsigned char *)pu8Str, u8LineNumber, u16Size, 0, u32TextColor, UPD_BACK_GROUND_COLOR, u8Posx, POS_TOP);
